...
model = dict(
    type='DynamicMVXFasterRCNN',#' MaskRCNN',
    img_backbone=dict(#backbone提特征
        type='ResNet',
        depth=50,
        num_stages=4,#输出几个特征图
        out_indices=(0, 1, 2, 3),#金字塔，将4个特征图都输出
        frozen_stages=1,
...
train_pipeline = [
    ...
    dict(
        type='Resize',
        img_scale=[(640, 192), (2560, 768)],#如果内存不够可以调整这里的大小
...
data = dict(
    samples_per_gpu=2,#如果是单卡，当作是batchsize，如果是多卡，既每张卡的batchsize
    workers_per_gpu=2,
    train=dict(
        type='RepeatDataset',
        times=2,
        dataset=dict(
            type='KittiDataset',
            data_root='data/kitti/',#数据
            ann_file='data/kitti/kitti_infos_train.pkl',#标签
            split='training',
    ...
    test=dict(
        ...
        box_type_3d='LiDAR'))
evaluation = dict(
    interval=10,#经过一个epoch后便走一遍验证集，可以设置大些，
    pipeline=[
        dict(
            type='LoadPointsFromFile',
            coord_type='LIDAR',
